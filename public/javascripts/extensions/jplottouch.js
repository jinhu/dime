jQuery.fn.drawData=	function (context,data){
	var step=10;
	var offset=0;
	var scale=1;
...
jQuery.fn.fillRoundedRect=	function (context, x,y,w,h,r){
...
jQuery.fn.drawLables=	function (context,unit){
		context.font="12pt Arial";
		context.strokeStyle="hsl(222, 50%, 76%)";
		context.strokeText(unit, 250,30);
		var step=45;
...
jQuery.fn.drawAxes=	function (context){
...
jQuery.fn.plot_glucose = function(data){
	...
	var offset=0;
	var scale=1;
	this.fillRoundedRect(context,0,0,300,300,30);
	this.drawAxes(context);
	this.drawLables(context,"pmol/L");
	this.drawData(context,data.points);
}
jQuery.fn.plot_dl = function(data, config){
	if (!this[0].getContext){  
		console.log("The supplied HTML element does not appear to be a canvas");
		return;
	}
	...
	this.fillRoundedRect(context,0,0,300,300,30);
	this.drawAxes(context);
	this.drawLables(context,"mmol/L");
	this.drawData(context,data.sbp);
	this.drawData(context,data.dbp);
}
jQuery.fn.plot_bp = function(data, config){
	...
	this.fillRoundedRect(context,0,0,300,300,30);
	this.drawAxes(context);
	this.drawLables(context,"mmHg");
	this.drawData(context,data.sbp);
	this.drawData(context,data.dbp);
}
jQuery.fn.plot = function(data, config){
	if (!this[0].getContext){  
		console.log("The supplied HTML element does not appear to be a canvas");
		return;
	}
